...
-- 1. 删除相关索引
DROP INDEX IF EXISTS idx_attachments_email_part;
DROP INDEX IF EXISTS idx_attachments_part_id;
...
CREATE TABLE IF NOT EXISTS attachments_temp (
    ...
    -- 编码信息
    encoding VARCHAR(50) NOT NULL DEFAULT '7bit',
...
-- 3. 复制数据（不包括part_id列）
INSERT INTO attachments_temp (id, email_id, filename, content_type, size, content_id, disposition, file_path, is_inline, is_downloaded, encoding, user_id, created_at, updated_at, deleted_at)
SELECT id, email_id, filename, content_type, size, content_id, disposition, file_path, is_inline, is_downloaded, encoding, user_id, created_at, updated_at, deleted_at
FROM attachments;
